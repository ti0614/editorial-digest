...
- 各紙が発行元サイトで「鍵アイコン」等により会員限定を明示している場合、
  ...
  実機確認済みです。会員限定と判明したのは以下の8紙で、それ以外の紙は
  一覧ページ・記事ページ上に会員限定を示す表示が無かった（＝無料で読める）
  ことを確認しています。
  - 福島民報・神奈川新聞・北國新聞・熊本日日新聞・茨城新聞・南日本新聞：
    掲載分すべてが会員限定（一覧ページ上に鍵アイコン等の目印あり）
  - 北日本新聞：掲載分すべてが会員限定。ただし一覧ページ自体には目印が
    無く、記事個別ページを開いて初めて「ニュースプラン」会員登録が
    必要と分かる（`always_paid: true` で常時会員限定扱い）。
  - 沖縄タイムス：掲載から一定期間が経つと会員限定に切り替わる（直近の
    記事は無料）
  - サイト側の仕様変更で表示が変わる可能性はあるため、定期的な再確認を
    推奨します。
