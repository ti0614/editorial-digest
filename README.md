...
- 各紙が発行元サイトで「鍵アイコン」等により会員限定を明示している場合、
  ...
  実機確認済みです。会員限定と判明したのは以下の15紙で、それ以外の紙は
  ...
  - 北日本新聞・山形新聞・愛媛新聞・大分合同新聞・宮崎日日新聞・
    山陰中央新報・徳島新聞・千葉日報：掲載分すべてが会員限定。ただし
    一覧ページ自体には目印が無く、記事個別ページを開いて初めて（本文が
    有料案内に置き換わる、冒頭数行だけで「全文を読むには会員登録」と
    表示される、または「残り◯◯文字」と残り文字数が明示される）会員限定
    と分かる（`always_paid: true` で常時会員限定扱い）。千葉日報のみ
    一覧ページに「有料記事」アイコンの目印があった。
